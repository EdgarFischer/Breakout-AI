...
class Ball(Turtle):
    ...
    def wall_collision(self,HGrid,VGrid,YPAD): #check if the ball would collide with the wall and adjust speed and position in this case note that x has to be between -350 and 350 and y cannot exceed 275
        A= True
        ...
        elif self.xcor()+self.vx*MOVE >MAX:
            self.setx(MAX)
            self.vx=-1*self.vx # reverse x velocity
        elif self.ycor()+self.vy*MOVE > MAXY:
            self.sety(MAXY)
            self.vy=-1*self.vy # reverse y velocity
        elif self.ycor()+self.vy*MOVE < MINY:
            self.vx = random.randint(-2, 2) # random initial x velocity between -2 and 2
            self.vy = 1 # the initial y velocity is always exactly 1
            self.setx(0)
            self.sety(YPAD+50)
    ...
    def paddle_collision(self, paddle, YPAD): #check if the ball would collide with the paddle and adjust speed and position in this case
        ...
        if self.ycor() < YPAD +100 and abs(self.xcor()+50 - P1)<35  and self.vx > 0: #does it hit left pixel?
            self.vy = 1
            self.vx = -2 
        elif self.ycor() < YPAD +100 and abs(self.xcor() - P1)<35: #does it hit left pixel?
            self.vy = 1
            self.vx = -2 
        elif self.ycor() < YPAD +100 and abs(self.xcor() - P2)<35:
            self.vy = 1
            self.vx = -1
        elif self.ycor() < YPAD +100 and abs(self.xcor() - P3)<35:
            self.vy = 1
            self.vx = 0
        elif self.ycor() < YPAD +100 and abs(self.xcor() - P4)<35:
            self.vy = 1
            self.vx = 1   
        elif self.ycor() < YPAD +100 and abs(self.xcor() - P5)<35:
            self.vy = 1
            self.vx = 2
        elif self.ycor() < YPAD +100 and abs(self.xcor()-50 - P5)<35 and self.vx < 0 :
            self.vy = 1
            self.vx = 2
        else:
            A=False 
    ...
    def update(self, paddle, HGrid,VGrid,YPAD): # this function defines the position update and also detects possible collisions
        ...
        if A == False:
            B = self.wall_collision(HGrid,VGrid,YPAD)
